...
[task_local]
#京喜财富岛合成月饼
5 * * * * https://raw.githubusercontent.com/Aa123on-lv/sync/jd_scripts/jd_cfd_mooncake.js, tag=京喜财富岛合成月饼, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jxcfd.png, enabled=true

================Loon==============
[Script]
cron "5 * * * *" script-path=https://raw.githubusercontent.com/Aa123on-lv/sync/jd_scripts/jd_cfd_mooncake.js,tag=京喜财富岛合成月饼

===============Surge=================
京喜财富岛合成月饼 = type=cron,cronexp="5 * * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/Aa123on-lv/sync/jd_scripts/jd_cfd_mooncake.js

============小火箭=========
京喜财富岛合成月饼 = type=cron,script-path=https://raw.githubusercontent.com/Aa123on-lv/sync/jd_scripts/jd_cfd_mooncake.js, cronexpr="5 * * * *", timeout=3600, enable=true
 */
...
!(async () => {
  ...
  let res = await getAuthorShareCode('https://raw.githubusercontent.com/Aa123on-lv/updateTeam/master/shareCodes/cfd.json')
  if (!res) {
    $.http.get({url: 'https://purge.jsdeliv123r.net/gh/Aa123on-lv/updateTeam@master/shareCodes/cfd.json'}).then((resp) => {}).catch((e) => console.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await getAuthorShareCode('https://cdn.jsdeliv123r.net/gh/Aa123on-lv/updateTeam@master/shareCodes/cfd.json')
  }
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `http://transfer.n1z.lu/cfd`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
