...
!(async () => {
    ...
    let res = [];
    try{res = await getAuthorShareCode('https://raw.githubusercontent.com/ls123h26/share_code/main/shop.json');}catch (e) {}
    if(!res){
        try{res = await getAuthorShareCode('https://raw.fastgit.org/ls123h26/share_code/main/shop.json');}catch (e) {}
        if(!res){res = [];}
    }
    let res2 = [];
    try{res2 = await getAuthorShareCode('https://raw.githubusercontent.com/zer123o205/updateTeam/main/shareCodes/shop.json');}catch (e) {}
    if(!res2){
        try{res2 = await getAuthorShareCode('https://raw.fastgit.org/ze123ro205/updateTeam/main/shareCodes/shop.json');}catch (e) {}
        if(!res2){res2 = [];}
    }
    res = [...res,...res2]
    if(res.length > 0){
