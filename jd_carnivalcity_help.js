...
!(async () => {
  ...
  $.temp = [];
  $.updatePkActivityIdRes = await getAuthorShareCode('https://raw.githubusercontent.com/Aa123ron-lv/updateTeam/master/shareCodes/jd_cityShareCodes.json')
  if (!$.updatePkActivityIdRes) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/Aa123ron-lv/updateTeam@master/shareCodes/jd_cityShareCodes.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.updatePkActivityIdRes = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/Aar123on-lv/updateTeam@master/shareCodes/jd_cityShareCodes.json')
  }
  await requireConfig();
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `http://share.turing123labs.net/api/v3/carnivalcity/query/20/`, 'timeout': 20000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
